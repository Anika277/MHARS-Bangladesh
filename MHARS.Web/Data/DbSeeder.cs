...
public static class DbSeeder
{
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        // MigrateAsync() moved to Program.cs so this method only seeds data.
            ...
            if (!await roleManager.RoleExistsAsync(role))
            {
                ...
                if (!roleResult.Succeeded)
                    throw new InvalidOperationException(
                        $"Role '{role}' seed failed: {Describe(roleResult)}");
            }
        }

        // ---------- Admin account ----------
        // Demo credentials only. Must not ship to production.
        ...
        if (await userManager.FindByEmailAsync(adminEmail) is null)
        {
            ...
            // Without this check, a failed CreateAsync would still fall through to
            // AddToRoleAsync and cause a foreign key violation on AspNetUserRoles.
            var createResult = await userManager.CreateAsync(admin, "Admin@123");
            if (!createResult.Succeeded)
                throw new InvalidOperationException(
                    $"Admin user seed failed: {Describe(createResult)}");

            var roleAssign = await userManager.AddToRoleAsync(admin, "Admin");
            if (!roleAssign.Succeeded)
                throw new InvalidOperationException(
                    $"Admin role assignment failed: {Describe(roleAssign)}");
        }
        ...
        if (!await db.Shelters.AnyAsync())
        {
            db.Shelters.AddRange(
                ...
                // Sirajganj and Sunamganj had alerts but no shelters, so the
                // district text-match lookup returned an empty list for them.
                new Shelter { Name = "Sirajganj Govt. High School",         District = "Sirajganj",   Address = "S.S. Road, Sirajganj Sadar",    Capacity = 850,  ContactNumber = "0751-62013" },
                new Shelter { Name = "Sunamganj Govt. Jubilee High School", District = "Sunamganj",   Address = "Hasan Nagar, Sunamganj Sadar",  Capacity = 750,  ContactNumber = "0871-55029" }
            );
        }
        ...
        {
            // IssuedAt was never set, so every row defaulted to 0001-01-01.
            // Offsets from one reference instant keep "latest first" meaningful.
            var now = DateTime.UtcNow;

            db.Alerts.AddRange(
                new Alert
                {
                    HazardType = HazardType.Flood, District = "Sirajganj",
                    Title = "Jamuna River Inundation",
                    Message = "Water level at Jamuna River Sirajganj point is 13.83m (+48cm over danger mark). Embankment seepage reported at Kazipur and Belkuchi. Residents of char areas are advised to relocate to the nearest shelter.",
                    Severity = SeverityLevel.High, SourceReference = "FFWC Bulletin #48",
                    IssuedBy = "Authority", IssuedAt = now.AddHours(-3)
                },
                new Alert
                {
                    HazardType = HazardType.Flood, District = "Kurigram",
                    Title = "Brahmaputra Overflow",
                    Message = "Brahmaputra at Chilmari flowing +52cm above danger level. Chilmari and Nageshwari lowlands flooded across 15 unions. Safe drinking water tube-wells submerged.",
                    Severity = SeverityLevel.High, SourceReference = "FFWC Bulletin #47",
                    IssuedBy = "Authority", IssuedAt = now.AddHours(-7)
                },
                new Alert
                {
                    HazardType = HazardType.Earthquake, District = "Sylhet",
                    Title = "M 4.6 Seismic Tremor",
                    Message = "Epicenter 38km ESE of Sylhet at 12.4km depth. Tremor felt across Sylhet city. No damage reported. Aftershocks possible.",
                    Severity = SeverityLevel.Medium, SourceReference = "USGS",
                    IssuedBy = "USGS Auto", IssuedAt = now.AddHours(-14)
                },
                new Alert
                {
                    HazardType = HazardType.Flood, District = "Sunamganj",
                    Title = "Surma River Surge",
                    Message = "Surma River rising +12cm in 4 hours due to Meghalaya runoff. Haor basin residents advised to stay alert.",
                    Severity = SeverityLevel.Medium, SourceReference = "FFWC Bulletin #46",
                    IssuedBy = "Authority", IssuedAt = now.AddHours(-26)
                },
                new Alert
                {
                    HazardType = HazardType.Flood, District = "Dhaka",
                    Title = "Normal Baseline",
                    Message = "Buriganga and Turag rivers within normal seasonal limits. No action required.",
                    Severity = SeverityLevel.Low, SourceReference = "FFWC",
                    IssuedBy = "Authority", IssuedAt = now.AddHours(-50)
                }
            );
        }
