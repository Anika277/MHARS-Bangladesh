...
Put the canonical district list somewhere both you and Anika (Shelter) and Erin (dropdowns) can reference — a static `List<string>` in a shared `Constants/Districts.cs` is enough; don't let three people hand-type slightly different spellings.
...
- [ ] Verify soft-deleted (`IsActive = false`) alerts don't appear in citizen views but do appear in Admin's analytics history for Anika's dashboard.
